...
int main()
{
    ...
    while (first) {
        second = 1;
        printf("\nВыберите два города из списка. Города обязательно должны "
               "быть разными!\n");
        printtown();
        printf("\n");
        ...
        while (check_for_exist(arrival, g->name) == -1
               ...
               || !strcmp(arrival, departure)) {
            if (!strcmp(arrival, departure)
                && check_for_exist(departure, g->name) != -1
                && check_for_exist(arrival, g->name) != -1) {
            ...
            if (check_for_exist(arrival, g->name) == -1
                ...
                for (int i = 0; i < 20; i++) {
                    ...
                    check_for_exist(arrival, g->name) > -1
                    && check_for_exist(departure, g->name) == -1) {
                printf("Город прибытия %s отсутствует в списке городов "
                       "программы.\n",
                       departure);
                for (int i = 0; i < 20; i++) {
                    departure[i] = '\0';
        ...
        int start = check_for_exist(departure, g->name),
            end = check_for_exist(arrival, g->name), way = 0;
        int ways[20][V];
        char* answers[13];
        char ans[45];
        for (int i = 0; i <= 13; i++) {
        ...
        while (second) {
            ...
            printf("\nВведите номер или название выбранной функции из данного "
                   "списка: ");
            fgets(ans, 90, stdin);
            if (check_for_exist(ans, answers) == 0
                || !strcmp(ans, answers[6])) {
                command_for_search(g, start, end, ways, &way);
                printf("\n");
                print_shortest_way(g, ways, way, end);
                way = 0;
            } else if (
                    check_for_exist(ans, answers) == 1
                    || !strcmp(ans, answers[7])) {
                command_for_search(g, start, end, ways, &way);
                printf("\n");
                print_longest_way(g, ways, way, end);
                way = 0;
            } else if (
                    check_for_exist(ans, answers) == 2
                    || !strcmp(ans, answers[8])) {
                command_for_search(g, start, end, ways, &way);
                printf("\n");
                print_all_ways(g, ways, way, end);
                way = 0;
            } else if (
                    check_for_exist(ans, answers) == 3
                    || !strcmp(ans, answers[9])) {
                command_for_search(g, start, end, ways, &way);
                printf("\n");
                print_all_ways(g, ways, way, end);
                printf("\n");
                ...
                way = 0;
                    ...
                    check_for_exist(ans, answers) == 4
                    || !strcmp(ans, answers[10])) {
                second = 0;
            } else if (
                    check_for_exist(ans, answers) == 5
                    || !strcmp(ans, answers[11])) {
